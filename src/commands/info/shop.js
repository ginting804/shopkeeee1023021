        ...
        db.find({ id: message.guild.id }, async(err, data) => {
            ...
            if(data.length > 10) {
                ...
                for(const item of items) {
                    ...
                    arry.push(
                        new MessageEmbed()
                        .setTitle('Shop de ' + message.guild.name).setColor('RANDOM').setThumbnail(message.guild.iconURL({ dynamic: true }))
                ...
                message.channel.send(
                    new MessageEmbed()
                        .setTitle('Shop de '+ message.guild.name).setThumbnail(message.guild.iconURL({ dynamic: true })).setColor('RANDOM')
                        .setDescription(mapping)
                )
            } else if(data.length === 0) {
                message.channel.send('Aucun item dans le shop !')
            }
        })
      }
    }
