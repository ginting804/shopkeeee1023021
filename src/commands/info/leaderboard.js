        ...
        data.find({ Guild: message.guild.id }, async(err, data) => {
            ...
            if(data.length > 10 ){
                ...
                for(const chunk of chunks) {
                    ...
                    arry.push(
                        new MessageEmbed()
                        .setTitle('Leaderboard de ' + message.guild.name).setColor('RANDOM').setThumbnail(message.guild.iconURL({ dynamic: true }))
                ...
                message.channel.send(
                    new MessageEmbed()
                        .setTitle('Leaderboard de '+ message.guild.name).setThumbnail(message.guild.iconURL({ dynamic: true })).setColor('RANDOM')
                        .setDescription(mapping)
                )
            } else if(data.length === 0) {
                message.channel.send('Aucune personne dans le leaderboard !')
            }
        })
    }
}
