...
module.exports = {
    ...
    description: 'Permet de recommencer la cours financière des membres du serveur',
    usage: '',
    ...
    run: async(client, message, args) => {
        ...
        users.find({ Guild: message.guild.id  }, async(err, data) => {
            if(!data) {
                ...
                await message.channel.send(`Êtes-vous sur de vouloir réinitialiser les portes-feuilles ?`).then(async function (m) {
                    await m.react('✅');
                    await m.react('❌');
                ...
                collector.on('collect', async(reaction) => {
                    if(reaction.emoji.name == '✅') {
                        ...
                        await m.edit(`La cours financière a bien été réinitialisée !`)
                        collector.stop()
                    } else 
                    if(reaction.emoji.name == '❌') {
                        await m.edit(`La cours financière n'a pas été réinitialisée !`)
                        collector.stop()
                    }
                })
